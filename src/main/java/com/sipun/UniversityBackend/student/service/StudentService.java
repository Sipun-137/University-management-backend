...
//            String tempPassword = generateTempPassword();
            ...
            User user = User.builder().email(dto.getEmail()).password(encoder.encode(tempPassword)).role(User.Role.STUDENT).build();
            User savedUser = userRepo.save(user);


            int currentAcademicYear = getCurrentAcademicYear();

            Course course = courseRepo.findById(dto.getCourseId()).orElseThrow(() -> new ResourceNotFoundException("Course not found"));
            Branch branch = branchRepo.findById(dto.getBranchId()).orElseThrow(() -> new ResourceNotFoundException("Branch not found"));

            Batch batch = batchRepo.findByCourseAndBranchAndStartYear(course, branch, currentAcademicYear).orElseThrow(() -> new ResourceNotFoundException("No batch found for " + course.getName() + " - " + branch.getName() + " - " + currentAcademicYear));
            ...
            Section section = sections.stream().filter(s -> s.getCurrentStrength() < s.getMaxStrength()).min(Comparator.comparingInt(Section::getCurrentStrength)).orElseThrow(() -> new ConflictException("No available sections in batch/branch"));

            // Update section's current strength
            section.setCurrentStrength(section.getCurrentStrength() + 1);
            sectionRepo.save(section);
            int currentSemesterNumber = 1;
            Semester semester = semesterRepo.findByBranchAndNumber(branch, currentSemesterNumber).orElseGet(() -> {
                Integer maxSemester = semesterRepo.findMaxNumberByBranch(branch);
                return semesterRepo.findByBranchAndNumber(branch, maxSemester).orElseThrow(() -> new ResourceNotFoundException("No semesters available"));
            });

            // Create Student (INACTIVE status)
            Student student = Student.builder().name(dto.getName()).email(dto.getEmail()).phone(dto.getPhone()).emergencyContact(dto.getEmergencyContact()).gender(dto.getGender()).address(dto.getAddress()).dob(dto.getDob()).regdNo(regdNo).rollNo(rollNo).batch(batch).section(section).currentSemester(semester).branch(branch).user(savedUser).academicStatus(Student.AcademicStatus.ACTIVE).bloodGroup(dto.getBloodGroup()).nationality(dto.getNationality()).guardian(Student.Guardian.builder().guardianName(dto.getGuardian().getGuardianName()).guardianPhone(dto.getGuardian().getGuardianPhone()).relationship(dto.getGuardian().getRelationship()).build()).build();
    ...
    public StudentResponseDTO assignSection(Long studentId, SectionAssignmentDTO dto) {
        ...
        Section section = sectionRepo.findById(dto.getSectionId()).orElseThrow(() -> new ResourceNotFoundException("Section not found"));
        if (!section.getBatch().equals(student.getBatch()) || !section.getBranch().equals(student.getBranch())) {
            throw new ConflictException("Section does not match student's batch/branch");
        }

        // Validate semester
        Semester semester = semesterRepo.findById(dto.getSemesterId()).orElseThrow(() -> new ResourceNotFoundException("Semester not found"));
    ...
    private StudentResponseDTO convertToMinimalResponseDTO(Student student) {
        return StudentResponseDTO.builder().id(student.getId()).email(student.getEmail()).academicStatus(student.getAcademicStatus().name()).build();
    }

    private StudentResponseDTO convertToStudentResponseDTO(Student student) {
        return StudentResponseDTO.builder().id(student.getId()).name(student.getName()).email(student.getEmail()).phone(student.getPhone()).dob(student.getDob()).address(student.getAddress()).regdNo(student.getRegdNo()).rollNo(student.getRollNo()).course(student.getBranch().getCourse().getName()).branch(new BranchMinimalDTO(student.getBranch())).batch(new BatchMinimalDTO(student.getBatch())).section(student.getSection() != null ? student.getSection().getName() : "Not assigned").currentSemester(student.getCurrentSemester() != null ? "Semester " + student.getCurrentSemester().getNumber() : "Not assigned").academicStatus(student.getAcademicStatus().name()).guardian(new GuardianDTO(student.getGuardian().getGuardianName(), student.getGuardian().getGuardianPhone(), student.getGuardian().getRelationship())).nationality(student.getNationality()).emergencyContact(student.getEmergencyContact()).bloodGroup(student.getBloodGroup()).gender(student.getGender().name()).profilePhotoUrl(student.getProfilePhotoUrl()).build();
    }
    ...
    public void updateStudent(Long id, StudentUpdateDTO dto) {
        Student student = studentRepo.findById(id).orElseThrow(() -> new RuntimeException("Student not found"));
